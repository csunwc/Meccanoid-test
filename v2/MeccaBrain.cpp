...
byte MeccaBrain :: getServoPosition(int servoNum){
  ...
  if(moduleType[servoNum] == 'S'){
        ...
        // map servo # to module #, note moduleNum is next module, thus -1 to get current input
        if (moduleNum > 0){  // if next = 1..3 
          temp = moduleNum - 1;
        }
        else{  // next = 0
          //temp = 0;
        ...
        if(temp == servoNum){ // current input is wanted
          return inputByte;
        }
    }
...
void MeccaBrain :: communicate(){
    ...
    if (inputByte == 0xFE){                 // if received 0xFE "exist", then request module type 
      outputByte[moduleNum] = 0xFC;         // mark output byte for this module number for next invocation
    }
  
    if (inputByte == 0x01 && moduleType[moduleNum] == '_'){  // if received  0x01 (module type is a servo), change color to Blue
    	outputByte[moduleNum] = 0xF4;  // discovered
    	moduleType[moduleNum] = 'S';
    }

	/*  // toggle LED order LEDoutputByte1   ledOrder ???? 
  ...
  */
    ...
    if (inputByte == 0x02 && moduleType[moduleNum] == '_'){  // if received 0x02 (module ID is a LED),  color to Blue
        LEDoutputByte1 = 0x00;
        LEDoutputByte2 = 0x47;  
    	  outputByte[moduleNum] = LEDoutputByte1;  
    	  ledOrder = 1;
...
/*
    if(inputByte == 0x00){   // if received x00, reset the chain to be rediscovered.  Why ???
  ...
  */
    ...
    printModNum = moduleNum;      // save prev module #
    moduleNum++;                             // increment to next module ID
    if (moduleNum > 3) {  // loop 0-3 by reset to module 0
      	moduleNum = 0;
    } 
...
// returns inputByte , boolean determines if we want to advance to next module in chain
void MeccaBrain :: sendPacket(boolean advance){
    // send 6-byte packet to chain    
    sendByte(0xFF);                         // send header
 ...
 if (advance) {   // move up to next in chain
    printModNum = moduleNum;      // save prev module #
    moduleNum++;                             // increment to next module ID
    if (moduleNum > 3) {  // loop 0-3 by reset to module 0
        moduleNum = 0;
...
// find all modules in the chain in one pass
void MeccaBrain :: discoverModules(){
    ...
    if (inputByte == 0xFE)  {                 // if received 0xFE "exist", then request module type 
      outputByte[moduleNum] = 0xFC;         // mark output byte for this module number for next invocation
      // 2nd request - what type ?
      sendPacket(false);
          if (inputByte == 0x01 && moduleType[moduleNum] == '_') {  // if received  0x01 (servo)
            outputByte[moduleNum] = 0xF4;  // discovered
            moduleType[moduleNum] = 'S';
            }
          if (inputByte == 0x02 && moduleType[moduleNum] == '_') {  // if received 0x02 (LED)
            outputByte[moduleNum] = LEDoutputByte1;  //why???
            moduleType[moduleNum] = 'L'; 
            }
      }      
